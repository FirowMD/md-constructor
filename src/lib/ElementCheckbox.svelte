<script lang="ts">
  ...
  function handleCheck(value: string, checked: boolean) {
    ...
    if (elementIndex !== -1) {
      const updatedElement = { ...element };
      if (checked) {
        updatedElement.chosens = [...updatedElement.chosens, value];
      } else {
        updatedElement.chosens = updatedElement.chosens.filter((v: string) => v !== value);
      }
  ...
  function removeOption(index: number) {
    ...
    if (elementIndex !== -1) {
      ...
      updatedElement.chosens = updatedElement.chosens.filter((v: string) => v !== element.values[index]);
          ...
          <div class="flex items-center justify-between">
            <label class="flex items-center space-x-2">
              <input 
                class="checkbox" 
                type="checkbox" 
                checked={element.chosens.includes(value)}
                oninput={(e) => handleCheck(value, e.currentTarget.checked)}
              />
